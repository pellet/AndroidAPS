...
class OpenAPSaiSMBPlugin @Inject constructor(
    ...
    .fragmentClass(OpenAPSaiSMBFragment::class.java.name)
    ...
    override var lastAPSResult: DetermineBasalResultaiSMB? = null
    ...
    override fun invoke(initiator: String, tempBasalFallback: Boolean) {
        ...
        lastAPSResult = null
        val determineBasalAdapter = DetermineBasalAdapteraiSMB(context, injector)
        ...
        try {
            determineBasalAdapter.setData(profile, maxIob, maxBasal, minBg, maxBg, targetBg, activePlugin.activePump.baseBasalRate, iobArray, glucoseStatus, mealData,
        ...
        val determineBasalResult = determineBasalAdapter.invoke()
        profiler.log(LTag.APS, "AMA calculation", start)

        if (determineBasalResult.rate == 0.0 && determineBasalResult.duration == 0 && iobCobCalculator.getTempBasalIncludingConvertedExtended(dateUtil.now()) == null) determineBasalResult.tempBasalRequested = false
        determineBasalResult.iob = iobArray[0]
        val now = System.currentTimeMillis()
        determineBasalResult.json?.put("timestamp", dateUtil.toISOString(now))
        determineBasalResult.inputConstraints = inputConstraints
        lastDetermineBasalAdapter = determineBasalAdapter
        lastAPSResult = determineBasalResult as DetermineBasalResultaiSMB
        lastAPSRun = now
