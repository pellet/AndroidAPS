...
class OpenAPSaiSMBFragment : DaggerFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding.swipeRefresh) {
            ...
            setOnRefreshListener {
                ...
                Thread { openAPSaiSMBPlugin.invoke("OpenAPSaiSMB swiperefresh", false) }.start()
            }
        }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean =
        when (item.itemId) {
            ID_MENU_RUN -> {
                ...
                Thread { openAPSaiSMBPlugin.invoke("OpenAPSaiSMB menu", false) }.start()
                true
            }
    ...
    private fun updateGUI() {
        val naForAiSMB = "N/A for aiSMB"
        if (_binding == null) return
        openAPSaiSMBPlugin.lastAPSResult?.let { lastAPSResult ->
            binding.result.text = jsonFormatter.format(lastAPSResult.json)
            binding.request.text = lastAPSResult.toSpanned()
        }
        openAPSaiSMBPlugin.lastAPSResult?.let { lastResult ->
            binding.constraints.text = lastResult.constraintStr
            binding.glucosestatus.text = lastResult.glucoseStr
            binding.currenttemp.text = naForAiSMB
            binding.iobdata.text = lastResult.iobStr
            binding.profile.text = lastResult.profileStr
            binding.mealdata.text = lastResult.mealStr
            binding.scriptdebugdata.text = naForAiSMB
        }
        if (openAPSaiSMBPlugin.lastAPSRun != 0L) {
            binding.lastrun.text = dateUtil.dateAndTimeString(openAPSaiSMBPlugin.lastAPSRun)
        }

        binding.autosensdata.text = naForAiSMB
